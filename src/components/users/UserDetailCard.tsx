...
const UserDetailCard: React.FC<Props> = ({ user }) => {
  ...
  return (
    <div className="w-1/2 flex flex-col space-y-6 bg-gray-50 p-4 rounded-md shadow-md text-textPrimary">
      ...
      >
        <h2 className="text-sm font-semibold w-full">Ürün Kar Miktarı:</h2>
        <Input
      ...
      >
        <h2 className="text-sm font-semibold w-full">Kargo Kar Miktarı:</h2>
        <Input
      ...
      >
        <h2 className="text-sm font-semibold w-full">Ara Depo Kar Miktarı:</h2>
        <Input
      ...
      <div className="flex space-x-3 items-center">
        ...
        <button
          onClick={handleModal}
          className="bg-sageOrange text-white rounded-md py-1 px-4 hover:bg-orange-400"
        >
          Bakiye Düzenle
        </button>
      </div>
      <div className="flex space-x-3 items-center">
        ...
        {user?.stores.map((store) => <p key={store._id}>{store.storeName}</p>)}
      </div>
      <BalanceChangeModal
        isOpen={isModalOpen}
        userId={user?._id || ""}
