...
export default function LoginPage() {
	...
	const handleLogin = async (e: React.SyntheticEvent) => {
		...
		const result = await signIn("credentials", {
			redirect: false, // Sayfada kalmasını sağlamak için
			email,
			password,
			callbackUrl: "/",
		});
			...
			<div className="max-w-md w-full space-y-8">
				...
				<form className="mt-8 space-y-6" onSubmit={handleLogin}>
					...
					<div>
						<button
							type="submit"
							className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-primary hover:bg-secondary focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-secondary"
						>
							Login
						</button>
					</div>
